module MF::Import
  class MicrodataImporter < MF::Import::Importer
    ...
    def initialize(html, doc)
      super
      @doc = @doc.at('*[itemtype="http://data-vocabulary.org/Recipe"]')
    end
    
    def get_title
      @doc.at('*[itemprop=name]').inner_text.strip
    end
    
    def get_image_url
      el = @doc.at('*[itemprop=photo]')
    ...
    def get_ingredient_groups_with_depth(depth)
      ...
      parent = MF::Import::Importer.nearest_common_ancestor(@doc.search('*[itemprop=ingredient]'), depth)
      groups = []
    ...
    def try_get_directions
      directions = @doc.at('*[itemprop=instructions]').to_s
      other = []
      
      if el = @doc.at('*[itemprop=prepTime]')
        other << ["Prep Time", el.text]
      end
      
      if el = @doc.at('*[itemprop=cookTime]')
        other << ["Cook Time", el.text]
      end
      
      if el = @doc.at('*[itemprop=totalTime]')
        other << ["Total Time", el.text]
      end
      
      if el = @doc.at('*[itemprop=yield]')
        other << ["Yield", el.text]
      end
    ...
    def self.my_recipe?(url, doc)
      return doc.at('*[itemtype="http://data-vocabulary.org/Recipe"]')
    end
  end
end